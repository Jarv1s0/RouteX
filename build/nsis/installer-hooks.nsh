...
!macro ROUTEX_REINSTALL_SERVICE_AFTER_FILE_WRITE
  ${If} $ROUTEX_SERVICE_WAS_INSTALLED = 1
    Push $0
    Push $1
    Push $2
    DetailPrint "正在重新注册 RouteX 服务..."
    InitPluginsDir
    StrCpy $2 "$PLUGINSDIR\routex-reinstall-service.ps1"
    FileOpen $1 "$2" w
    FileWrite $1 "$$installDir = [System.IO.Path]::GetFullPath($$args[0])$\r$\n"
    FileWrite $1 "$$startAfterInstall = $$args[1] -eq '1'$\r$\n"
    FileWrite $1 "$$serviceBinary = Join-Path $$installDir '${ROUTEX_SERVICE_PATH}'$\r$\n"
    ...
    FileWrite $1 "if ($$startAfterInstall) {$\r$\n"
    ...
    Delete "$2"
    ${If} $0 != 0
      MessageBox MB_ICONEXCLAMATION|MB_OK "新版 RouteX 已安装，但 RouteX 服务重新注册失败。$\r$\n$\r$\n请打开 RouteX 后在“系统服务”中重新安装服务。"
    ${EndIf}
    Pop $2
    Pop $1
    Pop $0
...
!macro NSIS_HOOK_POSTINSTALL
  ...
  !else
    !insertmacro ROUTEX_REFRESH_SHORTCUT_ICON "$SMPROGRAMS\${ROUTEX_PRODUCT_NAME}.lnk"
  !endif
  !insertmacro ROUTEX_REINSTALL_SERVICE_AFTER_FILE_WRITE
!macroend

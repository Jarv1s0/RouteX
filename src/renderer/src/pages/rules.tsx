...
const formatQuickRule = (rule: Pick<QuickRule, 'type' | 'value' | 'target' | 'noResolve'>): string => {
...
const RulesPage: React.FC = () => {
  ...
  // 递归查找最终节点
  const getFinalNode = useCallback((proxyName: string, visited: Set<string> = new Set()): string | null => {
  ...
  const remoteItems = useMemo(() => {
    ...
    const firstSeenItems = new Map<string, {
    ...
    if (deferredFilter !== '') {
      return items.filter((item) => {
        ...
        const providerMatches = item.provider && (
                    ...
                    >
                      <CardBody className="w-full py-2 px-3">
                        <div className="flex items-center gap-2">
                          ...
                          />
                          <span className={`w-6 flex-shrink-0 -mr-1 text-xs text-foreground-400 ${!rule.enabled ? 'line-through' : ''}`}>
                            {index + 1}.
                          </span>

                          <div className={`flex min-w-0 flex-1 items-center ${!rule.enabled ? 'opacity-60 grayscale' : ''}`}>
                            {/* 类型 + 名称 + no-resolve */}
                            ...
                            <div className="flex flex-shrink-0 items-center gap-1 overflow-hidden">
                              ...
                              {getFinalNode(rule.target) && getFinalNode(rule.target) !== rule.target && (
                                <>
      ...
      {activeTab === 'remote' && (
        <div className="h-[calc(100vh-100px)]">
          ...
          <Virtuoso
            data={remoteItems}
            itemContent={(i, item) => {
              const isDisabled = item.originalIndex !== undefined 
              ...
              return (
                <RemoteRuleItem
                  ...
                  onToggle={item.originalIndex !== undefined ? () => toggleRule(item.originalIndex!) : undefined}
                  onUpdate={item.provider ? () => onUpdate(item.provider!.name) : undefined}
                  onView={item.provider ? () => {
                    setShowDetails({
                      show: false,

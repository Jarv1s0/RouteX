export const CONFIG_IPC_INVOKE_CHANNELS = {
  addChainItem: 'addChainItem',
  addOverrideItem: 'addOverrideItem',
  addProfileItem: 'addProfileItem',
  addQuickRule: 'addQuickRule',
  canRollbackOverride: 'canRollbackOverride',
  changeCurrentProfile: 'changeCurrentProfile',
  checkAutoRun: 'checkAutoRun',
  convertMrsRuleset: 'convertMrsRuleset',
  disableAutoRun: 'disableAutoRun',
  enableAutoRun: 'enableAutoRun',
  getAllChains: 'getAllChains',
  getAppConfig: 'getAppConfig',
  getChainsConfig: 'getChainsConfig',
  getControledMihomoConfig: 'getControledMihomoConfig',
  getCurrentProfileItem: 'getCurrentProfileItem',
  getFileStr: 'getFileStr',
  getOverride: 'getOverride',
  getOverrideConfig: 'getOverrideConfig',
  getOverrideItem: 'getOverrideItem',
  getProfileConfig: 'getProfileConfig',
  getProfileItem: 'getProfileItem',
  getProfileStr: 'getProfileStr',
  getQuickRules: 'getQuickRules',
  getQuickRulesConfig: 'getQuickRulesConfig',
  patchAppConfig: 'patchAppConfig',
  patchControledMihomoConfig: 'patchControledMihomoConfig',
  removeChainItem: 'removeChainItem',
  removeOverrideItem: 'removeOverrideItem',
  removeProfileItem: 'removeProfileItem',
  removeQuickRule: 'removeQuickRule',
  reorderQuickRules: 'reorderQuickRules',
  rollbackOverride: 'rollbackOverride',
  setActiveProfiles: 'setActiveProfiles',
  setFileStr: 'setFileStr',
  setOverride: 'setOverride',
  setOverrideConfig: 'setOverrideConfig',
  setProfileConfig: 'setProfileConfig',
  setProfileStr: 'setProfileStr',
  setQuickRulesConfig: 'setQuickRulesConfig',
  setQuickRulesEnabled: 'setQuickRulesEnabled',
  updateChainItem: 'updateChainItem',
  updateOverrideItem: 'updateOverrideItem',
  updateProfileItem: 'updateProfileItem',
  updateQuickRule: 'updateQuickRule',
  clearQuickRules: 'clearQuickRules'
} as const
